...
// Кастомний компонент іконки Instagram, оскільки він відсутній у базовому наборі lucide-react
...
// Централізований об'єкт стилів для легкого редагування класів Tailwind
const styles = {
  ...
  // Стилі для екранів завантаження та помилок
  ...
  // Навігація: збільшено z-index до 100 для перекриття всіх елементів
  nav: "fixed top-0 left-0 right-0 z-[100] transition-all duration-300 bg-white/90 backdrop-blur-md shadow-sm py-4",
  navTransparent: "fixed top-0 left-0 right-0 z-[100] transition-all duration-300 bg-transparent py-6",
  
  // Контейнер навігації зі збільшеними відступами на мобільних (px-6)
  navContainer: "container mx-auto px-6 md:px-12 lg:px-24 max-w-7xl flex justify-between items-center relative",
  ...
  // Посилання меню: жирність font-semibold для кращої читабельності
  navLink: "text-stone-600 hover:text-rose-400 font-semibold transition-colors cursor-pointer",
...
// Схема валідації форми контактів
...
// Компонент кнопки з варіантами стилів та анімацією натискання
...
// Універсальний компонент заголовка секції
...
export default function App() {
  // Стан для зберігання даних сайту
  const [data, setData] = useState(null);
  // Стан завантаження
  const [loading, setLoading] = useState(true);
  // Стан помилки
  const [error, setError] = useState(null);
  // Стан скролу для зміни фону меню
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        // Спроба завантажити data.json
        const response = await axios.get('/data.json');
        setData(response.data);
        setError(null);
      } catch (err) {
        console.error("Error fetching ", err);
        // Резервні дані, якщо файл не знайдено
        setData({
    ...
    // Обробник скролу для ефекту меню
    const handleScroll = () => setIsScrolled(window.scrollY > 20);
    window.addEventListener('scroll', handleScroll);
    return () => window.removeEventListener('scroll', handleScroll);
  }, []);

  // Функція плавного скролу до якоря
    ...
    <div className={styles.app}>
      ...
      <nav className={isScrolled ? styles.nav : styles.navTransparent}>
        <div className={styles.navContainer}>
          ...
          <button 
            className="md:hidden text-stone-800 p-2 z-[100] relative order-first" 
          ...
          {/* Пустий блок для центрування лого на мобільному */}
      ...
      <footer className={styles.footer}>
        <div className={styles.footerContainer}>
          {/* Зменшено жирність шрифту до font-medium */}
          <div className="text-3xl font-serif font-medium text-white">{profile.name}</div>
